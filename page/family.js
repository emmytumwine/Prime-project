document.addEventListener("DOMContentLoaded", function() {
  ...
  async function populateCategoryTypes() {
    try {
      const response = await fetch('/family/categoryTypes');
  ...
  async function fetchData() {
    ...
    const url = `/family/data?categoryType=${encodeURIComponent(categoryType)}`;
    const additionalDataUrl = `/family/additionalData?categoryType=${encodeURIComponent(categoryType)}`;
  ...
  function calculateResults() {
    ...
    if (statusType && categoryType && numberOfDirectParents >= 0 && numberOfDirectParentsInLaw >= 0 && numberOfChildren >= 0) {
      ...
      var apiUrl = `/family/additionalData?categoryType=${categoryType}`;
  ...
  const showCoverTableButton = document.getElementById("showCoverTableButton");
  const hideCoverTableButton = document.getElementById("hideCoverTableButton");
  const insuranceCoverTable = document.getElementById("dataContainer");
  
  // Hide the dataContainer and hideCoverTableButton initially
  insuranceCoverTable.style.display = "none";
  hideCoverTableButton.style.display = "none";
  
  // Event listener for showing the Insurance Cover Table
  showCoverTableButton.addEventListener("click", function () {
    insuranceCoverTable.style.display = "block";
    showCoverTableButton.style.display = "none";
    hideCoverTableButton.style.display = "block";
  
    // Automatically scroll down to the insuranceCoverTable
    insuranceCoverTable.scrollIntoView({ behavior: "smooth" });
  });
  
  // Event listener for hiding the Insurance Cover Table
  hideCoverTableButton.addEventListener("click", function () {
    insuranceCoverTable.style.display = "none";
    hideCoverTableButton.style.display = "none";
    showCoverTableButton.style.display = "block";
  });
